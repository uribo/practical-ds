...
```{r lp_missing_pattern, echo=TRUE, eval=FALSE, fig.height=8}
df_lp_kanto %>% 
  select(-starts_with(".")) %>% 
  vis_miss()
```
...
```{r lp_missing_pattern_upset, echo=TRUE, eval=FALSE}
df_lp_kanto %>% 
  select(-starts_with(".")) %>% 
  gg_miss_upset()
```
...
```{r lp_missing_pattern_facet, echo=TRUE, eval=FALSE, fig.height=8}
...
```{r hazard_kys_knn_impute, echo=TRUE, eval=FALSE, comment="8pt"}
p1 <- 
  ggplot() +
