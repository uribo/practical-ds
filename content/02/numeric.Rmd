...
```{r trans_logit, echo=TRUE, eval=FALSE}
df_transformed_examples %>% 
...
```{r linear_temp_and_expense, echo=TRUE, eval=FALSE}
beer_temperature_plot +
...
```{r polynomial_temp_and_expense, echo=TRUE, eval=FALSE}
p1 <- 
  beer_temperature_plot +
  geom_smooth(
    method = "lm",
...
```{r spline_temp_and_expense, echo=TRUE, eval=FALSE}
knots <- 
...
```{r plot_quantile, echo=FALSE, fig.width=6, fig.height=8, eval=FALSE}
set.seed(123)
...
```{r lp_categorize_distance, eval=FALSE, echo=TRUE}
df_lp_kanto_dist_c %>% 
...
```{r lp_interactions, eval=FALSE, echo=TRUE}
df_lp_kanto %>% 
